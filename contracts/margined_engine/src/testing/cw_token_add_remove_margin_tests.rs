...
fn test_add_margin_insufficent_balance() {
    ...
    let msg = engine
        .deposit_margin(vamm.addr().to_string(), 1, to_decimals(5001u64), vec![])
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
        err.source().unwrap().to_string(),
        format!(
            "Generic error: transfer failure - reply (id {})",
...
fn test_remove_margin() {
    ...
    let free_collateral = engine
        .get_free_collateral(&router.wrap(), vamm.addr().to_string(), 1)
        .unwrap();
...
fn test_remove_margin_insufficient_margin() {
    ...
    let msg = engine
        .withdraw_margin(vamm.addr().to_string(), 1, to_decimals(61u64))
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
        err.source().unwrap().to_string(),
...
fn test_remove_margin_incorrect_ratio_four_percent() {
    ...
    let msg = engine
        .withdraw_margin(vamm.addr().to_string(), 1, to_decimals(36u64))
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
        err.source().unwrap().to_string(),
...
fn test_remove_margin_unrealized_pnl_long_position_with_profit_using_spot_price() {
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(45_010_000_000u128),
        )
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(45_000_000_000u128),
        )
        .unwrap();
    router.execute(alice.clone(), msg).unwrap();
}
...
fn test_remove_margin_unrealized_pnl_long_position_with_loss_using_spot_price() {
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(24_900_000_000u128),
        )
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(24_850_746_257u128),
        )
        .unwrap();
    router.execute(alice.clone(), msg).unwrap();
}
...
fn test_remove_margin_unrealized_pnl_short_position_with_profit_using_spot_price() {
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(16_500_000_000u128),
        )
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(16_097_560_976u128),
        )
        .unwrap();
    router.execute(alice.clone(), msg).unwrap();
}
...
fn test_remove_margin_unrealized_pnl_short_position_with_loss_using_spot_price() {
    ...
    let msg = engine
        .withdraw_margin(vamm.addr().to_string(), 1, Uint128::from(2_500_000_000u128))
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
        err.source().unwrap().to_string(),
    ...
    let msg = engine
        .withdraw_margin(vamm.addr().to_string(), 1, Uint128::from(2_282_608_687u128))
        .unwrap();
    router.execute(alice.clone(), msg).unwrap();
}
...
fn test_remove_margin_unrealized_pnl_long_position_with_loss_using_twap_price() {
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(34_930_000_000u128),
        )
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(34_925_373_122u128),
        )
        .unwrap();
    router.execute(alice.clone(), msg).unwrap();
}
...
fn test_remove_margin_unrealized_pnl_short_position_with_profit_using_twap_price() {
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(15_600_000_000u128),
        )
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
    ...
    let msg = engine
        .withdraw_margin(
            vamm.addr().to_string(),
            1,
            Uint128::from(15_500_000_000u128),
        )
        .unwrap();
    router.execute(alice.clone(), msg).unwrap();
}
...
fn test_remove_margin_unrealized_pnl_short_position_with_loss_using_twap_price() {
    ...
    let msg = engine
        .withdraw_margin(vamm.addr().to_string(), 1, Uint128::from(8_700_000_000u128))
        .unwrap();
    let err = router.execute(alice.clone(), msg).unwrap_err();
    assert_eq!(
        err.source().unwrap().to_string(),
    ...
    let msg = engine
        .withdraw_margin(vamm.addr().to_string(), 1, Uint128::from(8_600_000_000u128))
        .unwrap();
    router.execute(alice.clone(), msg).unwrap();
}
