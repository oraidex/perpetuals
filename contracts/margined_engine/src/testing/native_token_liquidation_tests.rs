...
#[test]
fn test_liquidate_with_whitelisted_trader() {
    let mut env = new_native_token_scenario();
    ...
    // whitelist alice
    let msg = env.engine.add_whitelist(env.alice.to_string()).unwrap();
    env.router.execute(env.owner.clone(), msg).unwrap();
    ...
    let err = env
        .router
        .execute(env.bob.clone(), msg.clone())
        .unwrap_err();
    assert_eq!(
        err.source().unwrap().to_string(),
        "Generic error: trader is whitelisted".to_string()
    );
    // alice can liquidate this position
    env.router.execute(env.alice.clone(), msg).unwrap();
}
